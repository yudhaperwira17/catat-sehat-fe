...
import CreateData from '@/components/modal/input-user/create-monitor-postpartum.vue'
import Expand from '@/components/mother/ExpandPostPartum.vue'
import { useMonitorPostPartum, useReadDaysPostPartum } from '@/services/user-monitor-postpartum'
...
const { data: monitor, isLoading: loading, refetch } = useMonitorPostPartum(monitorParams)
...
const formattedMonitorData = computed(() => {
  ...
  const formatted = dataArray.map((item: any) => {
    return {
      ...
      dayName: item.dayPostPartum?.name || 'N/A',
      status: item.status,
...
<template>
  <div class="flex flex-col">
    <div class="flex md:flex-row md:items-center mb-6 flex-col gap-y-2 md:justify-between">
      <div>
        <h1 class="text-base font-semibold">Pemantauan Ibu Nifas</h1>
        <p class="text-gray-600 font-normal text-sm">Lembar Pemantauan Ibu Nifas</p>
      </div>
