...
const { mutate, isPending } = useAdminEditBloodSuplement(computed(() => props.id))
...
watchEffect(() => {
  if (checkupMother.value) {
    ...
    formData.value.monthId = checkupMother.value.monthBlood.id
    formData.value.date = DateTime.fromISO(checkupMother.value.date || '').toMillis()
    formData.value.note = checkupMother.value.note
