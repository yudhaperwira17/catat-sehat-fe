...
import { useElderlyDetail, useElderlyUpdate } from '../_services/elderly'
...
const { mutate, isPending } = useElderlyUpdate(computed(() => route.params.id) as Ref<string>)
const router = useRouter()

const { data: elderlyDetail } = useElderlyDetail(computed(() => route.params.id) as Ref<string>)
...
watch(
  ...
  (newData) => {
    if (newData) {
      formData.value = new CreateElderlyData({
        ...newData,
        dateOfBirth: DateTime.fromISO(newData.dateOfBirth).toMillis()
      })
    }
  },
  { immediate: true }
